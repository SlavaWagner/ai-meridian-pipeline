...
const DEFAULT_CONFIG = {
  ...
  clientSecret: '',
  refreshToken: '',
  googleAdsVersion: 'v16', // or active API version
  loginCustomerId: '',
  accessToken: '',
  tokenExpiry: 0,
...
export async function refreshAccessToken() {
  ...
  if (!config.refreshToken) {
    throw new Error('No OAuth2 refresh token found. Please run the setup command first.');
  }
  ...
  try {
    ...
    throw new Error(`Failed to refresh Google Ads OAuth2 token: ${errorDetails}`);
  }
}
